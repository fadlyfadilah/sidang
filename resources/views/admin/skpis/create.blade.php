        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.skpis.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group {{ $errors->has('nama') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="nama_id" id="nama_id" required>
                                @foreach($namas as $id => $entry)
                                    <option value="{{ $id }}" {{ old('nama_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('nama'))
                        ...
                        <div class="form-group {{ $errors->has('kualifikasi') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" type="text" name="kualifikasi" id="kualifikasi" value="{{ old('kualifikasi', '') }}" required></textarea>
